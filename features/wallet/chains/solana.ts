...
import {
  ...
  TokenInfo,
  ENV,
} from '@solana/spl-token-registry/dist/main/index';
import { Market, MARKETS, Orderbook, TOKEN_MINTS } from '@project-serum/serum';
import { ethers } from 'ethers';
import { Token } from '..';

export const solana = new solanaWeb3.Connection(
...
const getPrice = async (marketInfo) => {
  ...
  const [bids, asks] = await Promise.all([
    market.loadBids(solana),
    market.loadAsks(solana),
  ]);
...
const getMarketInfo = (tokenAddresses) =>
  tokenAddresses.reduce((acc, tokenAddress) => {
    const SERUM_TOKEN = TOKEN_MINTS.find(
      (a) => a.address.toBase58() === tokenAddress,
    );
...
const getChainCoin = async (address, tokenList: TokenInfo[], markets) => {
  ...
  const userBalance = parseFloat(
    ethers.utils.formatUnits(balance, token.decimals).toString(),
  );
  return {
...
export const balanceOf = async ({ address }): Promise<Token[]> => {
  ...
  return Promise.all([
    ...
    ...tokens.value
      .map((t) => ({
        ...tokenList.find(
          (tl) => tl.address === t.account.data.parsed.info.mint,
        ),
