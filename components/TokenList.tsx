...
export const sumTokensBalanceUSD = (total, token: Token) =>
  (total += token.userBalanceUSD) || total;
const sortByUSDBalance = (a, b) =>
  a.userBalanceUSD < b.userBalanceUSD ? 1 : -1;
      ...
      ({ address, balances }) =>
        ...
        mergeTokens(
          Object.entries(balances).map(([chainId, { balances }]) => {
            if (addressesChainFilter[address]?.find((c) => c === chainId))
              return [];
            return balances;
          }),
        ),
...
export type TokenBalanceByChain = {
  [tokenContractAddress: string]: Record<
    ChainId,
    { [walletAddress: string]: Token }
  >;
};

const mapTokenBalanceChain = (
  tokens = [],
  addressesData: AddressData[],
): TokenBalanceByChain =>
  tokens.reduce(
    (accTokens, token) =>
      token && {
        ...accTokens,
        [token.contractAddress]: addressesData.reduce(
          (aAddrs, { address, balances }) => {
...
export const TokenList = ({
  ...
  if (!tokens || tokens.length === 0)
    return (
      <Text css={{ fontWeight: '$semibold', color: slate.slate10 }}>
        No tokens here yet
      </Text>
    );
  return (
    <>
      ...
      >
        <Text css={{ fontSize: '$lg', fontWeight: '$medium' }}>
          <Text css={{ fontWeight: '$extrabold', fontSize: '$lg' }}>
            Total Value:
          </Text>{' '}
