          ...
          <HoverCard.Trigger>
            <View
              css={{
                p: 5,
                borderRadius: '50%',
                backgroundColor: Chain[chainId].color,
              }}
            ></View>
          </HoverCard.Trigger>
...
export const TokenRow = ({ css = {}, token, tokenByChain, ...props }) => {
  const priceVariance24hr =
    ((token.priceUSD - token.priceUSD24HR) / token.priceUSD24HR) * 100;
  return (
    <View
      whileTap={{ scale: 0.95 }}
      css={{
      ...
      >
        <View css={{ alignItems: 'center' }}>
          ...
          <View css={{ flexDirection: 'column', ml: '1rem' }}>
            <Text
              css={{ fontSize: '$regular', fontWeight: '$bold', maxWidth: 200 }}
            >
              {token.name}
            </Text>
            <Text
