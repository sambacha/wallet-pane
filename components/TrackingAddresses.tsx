...
const TrackedAddress = ({
  ...
  const hasErrored =
    !isLoading && (addressData.isError || addressData.isRefetchError);
  ...
  const addressWithoutTokens =
    (!isLoading && chains.length === 0) || hasErrored;
          ...
          (addressWithoutTokens ? (
            ...
            <View
              css={{
                ...
                gap: 8,
                gridTemplateColumns:
                  'repeat(auto-fill, minmax(68px,min-content))',
              }}
            >
              {chains?.map((chain) => (
                <ChainBadge
                  ...
                  tokensValueUSD={balances[chain]?.balances?.reduce?.(
                    sumTokensBalanceUSD,
                    0,
                  )}
